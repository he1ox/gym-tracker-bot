    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card elev-sm" style={{ padding: 'var(--space-8)' }}>
        ...
        {model.isBodyweight ? (
          ...
          <p className="text-muted" style={{ margin: 0, fontSize: 13 }}>
            Aún no hay suficientes semanas registradas para mostrar una tendencia.
          </p>
        ) : (
          <AreaChart
        ...
        <div className="card elev-sm" style={{ padding: 'var(--space-8)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {model.sessionVolume.slice(0, 10).map((entry) => (
              <div key={entry.workoutId} style={{ display: 'grid', gridTemplateColumns: '110px 1fr 60px', gap: 10, alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 12, textAlign: 'right' }}>{entry.tonnageLabel}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {model.history.slice(0, 8).map((entry) => (
          <div key={entry.workoutId} className="card elev-sm" style={{ padding: 'var(--space-6) var(--space-8)' }}>
